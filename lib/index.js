...
/** True for addresses inside 100.64.0.0/10 — the Tailscale/CGNAT mesh range,
* never a real home/office LAN segment. */
function isMeshRange(address) {
...
/** LAN IPv4 addresses of this machine (non-internal interfaces, no mesh VPNs). */
function lanIpv4() {
  return Object.values(networkInterfaces())
    .flat()
    .filter((iface) => iface !== void 0 && iface.family === "IPv4" && !iface.internal)
    .map((iface) => iface.address)
    .filter((address) => !isMeshRange(address));
}
